...
const float INIT_PROB = 1.0; // Initial state probability of 1
...
class DirectedBondPercolation : public ECA {
    protected:
        bool getState(bool, bool, bool);
    public:
        using ECA::ECA;
...
// Directed Site Percolation

class DirectedSitePercolation : public ECA {
    protected:
        bool getState(bool, bool, bool);
    public:
        using ECA::ECA;
...
ECA::ECA(int s) : ECA(s, 0.0) {}

// Constructor
// Instantiates class with given size and probability
ECA::ECA(int s, float prob) {
    r250_init(SEED);
    probability = prob;
    dbpProb = 2.0*probability - probability*probability;
    size = s;
    state = new bool[size];
...
// Directed Bond Percolation
bool DirectedBondPercolation::getState(bool left, bool current, bool right) {
    if ((current != right) && ((dr250()/2.0) < probability)) {
        return true;
    }
    if ((current && right) && ((dr250()/2.0) < dbpProb)) {
        return true;
    }
    return false;
}
...
// Directed Site Percolation
bool DirectedSitePercolation::getState(bool left, bool current, bool right) {
    if ((current != right) && ((dr250()/2.0) < probability)) {
        return true;
    }
    if ((current && right) && ((dr250()/2.0) < probability)) {
